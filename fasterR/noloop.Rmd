...
The plyr package All plyr functions work based on the first two letters. The first denotes the input, the second the output.  For example, <span class="func">ldply</span> takes a list and converts the output to a data.frame.
...
We can potentially take advantage of the power of C++ explicitly within R, by using the Rcpp package. Many R packages do so.
...
matrixlm = microbenchmark(lm_base  = lm(y ~ x), 
                          ...
                          matrix_raw = solve(t(X) %*% X) %*% t(X) %*% y,
                          matrix_grouped = solve(t(X)%*%X) %*% (t(X) %*% y),
                          matrix_crossprod = crossprod(solve(crossprod(X)), crossprod(X,y)), times=10)

save(matrixlm, file='data/matrixlm.RData')
```

We can see above that these approaches all procuce the same results, but take very different amounts of time to complete. We get a significant gain if we reduce the lm overhead, where lm.fit is almost 10x faster. We can halve that by using matrix operations, and even get a slight gain if we group them a certain way.  This reduces memory usage as well.  The idea is that if we go from `solve(t(X)%*%X)` and post multiply by `t(X)`, we're dealing with a 2 x `r n` matrix. If we first `t(X) %*% y`, we're now just multiplying a 2x2 matrix with a 2x1 matrix.
...
[Microsoft Open R](https://mran.revolutionanalytics.com/open/) While the build is a bit behind, the current version of R
