...
This document provides ways to speed up your code *before* parallelization.  There are many ways to use R in an optimal fashion, but often these are not taught in applied courses or perhaps are not presented in a cohesive way.  In other cases, one may simply be new to R, or coming at it from a programming language that works differently.  
...
As a working example we'll start with a function that does a random walk, based on [this document](https://www.stat.auckland.ac.nz/~ihaka/downloads/Taupo-handouts.pdf), co-authored by one of the R's in R, with some slight additions and modifications. I like it because it demonstrates that sometimes even seemingly sequential operations might be put in a form that would not require a loop.  First we'll do things the hard way with a loop, and then show alternative methods later.
