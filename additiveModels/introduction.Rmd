...
<span class="marginnote">$$y\sim \mathcal{N}(\mu,\sigma^{2})$$  <br> $\mu = b_{0}+b_{1}X_{1}+b_{2}X_{2}...+b_{p}X_{p}$</span>
...
In that light, we may consider the *generalized* linear model.  Generalized linear models incorporate other types of distributions<label for="sn-demo" class="margin-toggle sidenote-number"></label><input type="checkbox" id="sn-demo" class="margin-toggle"/><span class="sidenote">Of the [exponential family](https://en.wikipedia.org/wiki/Exponential_family).</span>, and include a link function $g(.)$ relating the mean $\mu$, or stated differently, the estimated fitted values $E(y)$, to the linear predictor $X\beta$, often denoted $\eta$.  The general form is thus $g(\mu) = X\beta$. 
...
<span class="marginnote">
$$\mu = e^{b_{0}+b_{1}X_{1}+b_{2}X_{2}...+b_{p}X_{p}}$$
</span>
...
```{r fitPlotsDataSetup, cache=TRUE, echo=F}
...
```{r quadraticFitPlot, cache=F, echo=F}
plot_ly(data=dxy, x=x, y=y, mode='markers', showlegend=F, width='75%',
                    marker=list(opacity=.2)) %>% 
  add_trace(data=dxy, x=x, y=fitx2, name='lm fit') %>% 
  theme_plotly() %>% 
  layout(paper_bgcolor='rgba(0,0,0,0)',
         plot_bgcolor='rgba(0,0,0,0)')
```
...
```{r loessgraph1, echo=FALSE, cache=FALSE}
...
plot_ly(x=c(minx,minx), y=c(0, maxy), mode='line', showlegend=F, width='75%',
        line=list(color='gray'), opacity=.2, marker=list(opacity=0)) %>% # marker opacity to 0 to remove idiotic placed endpoints
  add_trace(x=c(maxx,maxx), y=c(0, maxy), mode='line', showlegend=F,
        line=list(color='gray'), opacity=.2, marker=list(opacity=0)) %>% 
  add_trace(data=dxy[-neighborhood,], x=x, y=y, mode='markers', showlegend=F, 
                    marker=list(color='gray', opacity=.2))%>% 
  add_trace(data=dxy[neighborhood,], x=x, y=y, mode='markers', showlegend=F, 
            marker=list(color='darkred', opacity=.2)) %>% 
  theme_plotly() %>% 
  layout(paper_bgcolor='rgba(0,0,0,0)',
         plot_bgcolor='rgba(0,0,0,0)')
```
...
```{r loessgraph2, echo=FALSE, cache=FALSE}
plot_ly(data=dxy[neighborhood,], x=x, y=y, mode='markers', showlegend=F, width='75%',
                    marker=list(opacity=.2)) %>% 
  add_trace(data=dxy[neighborhood,], x=x, y=neighborfit, name='lm fit') %>% 
  theme_plotly() %>% 
  layout(paper_bgcolor='rgba(0,0,0,0)',
         plot_bgcolor='rgba(0,0,0,0)')
```
...
```{r mcycleFigure, cache=F, echo=FALSE}
g = ggplot(aes(x=times,y=accel), data=mcycle3) + 
  geom_point(color='dodgerblue', alpha=.5) + 
  geom_line(aes(y=value), color='#ff5503', size=.75) +
  facet_wrap(~variable) +
  ylab('Acceleration') +
  xlab('Time') +
