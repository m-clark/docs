...
polynomialBasis = ggplot(aes(x=Income, y=Income), data=dpoly) +
  ...
  ylab("") +
  geom_smooth(aes(x=Income, y=Overall), se=F, method='lm', formula=y~poly(x,3), data = dpoly[261:325,], color="#2957FF", lwd=.5) +
...
csbyhand = ggplot(aes(x=Income, y=Overall), data=pisa)+
  ...
  geom_line(aes(x=xp, y=Xp%*%coef(mod.1)), data=data.frame(xp,Xp), color="#2957FF", lwd=.5) +
  xlim(.4,1) +
  theme_trueMinimal()
