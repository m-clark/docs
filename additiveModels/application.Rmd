...
g = ggplot(aes(x=Value,y=Overall), data=dmelt) +
...
ggplotly(g) %>% layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
```
...
g = ggplot(aes(x=Income, y=Income), data=dpoly) +
...
g = ggplot(aes(x=Income, y=Overall), data=d)+
...
# an example that will render;  using ribbon because plotly can't do stat/geom_smooth
g = ggplot(aes(x=Income,y=fit), data=predicts) +
  geom_point()
  # geom_ribbon(aes(ymin = lower, ymax=upper),
  #             fill='gray90') +
  # geom_line(color='#1e90ff')
ggplotly(g)  ##PLOTLY CURRENTLY BROKEN FOR THIS AND REMAINING PLOTS

# g = ggplot(predicts) + 
#   geom_ribbon(aes(ymin=lower, ymax=upper, x=Income), alpha=.1) +
#   geom_line(aes(x=Income, y=fit), color='#1e90ff') 
```
...
with(tt, plot_ly(x=Income, y=Edu,  z=Overall, type='heatmap', colors=colfun, hoverinfo='none'))
```
