...
In the case of neural net models, which have found a resurgence of interest of late, one can imagine a model where the input units (predictor variables) are  weighted and summed to create hidden layer units, which are then transformed and put through the same process to create outputs (see a simple example to the right).

<span class="marginnote"><img src='vis/nnet.png'> <br> <span style="text-align:center; disply:block; width:100px">A Neural Net Model</span><br><br></span>
