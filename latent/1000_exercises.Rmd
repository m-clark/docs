...
Note that these exercises are intended for those in the workshop first and forement. I provide them a separate notebook file
...
```{r pa1Hint1, echo=TRUE, eval=FALSE}
...
```{r pa1Hint2, echo=TRUE, eval=FALSE}
...
```{r echo=FALSE, results='hide', eval=-6}
satistifaction_ModelResults = sem(satistifaction_ModelCode, data=mtf)
summary(satistifaction_ModelResults, rsquare=T)
...
1. Run a factor analysis using the `'data/nlsy97_governmentNumeric.csv'` dataset.  Your first model will have all items (sans ID) loading on a single factor.  With <span class="pack">lavaan</span>, use the <span class="func">cfa</span> function.  Recall that you give it the model code (as a <span class="objclass">string</span>) and a `data = ` argument like `cfa(mod, data=mydata)`.
...
# Compare the two († means the better result); requires semTools package
summary(compareFit(model1, model2, nested=F))
```
