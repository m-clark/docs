...
```{r ivmodel2, echo=FALSE, eval=TRUE, cache=FALSE, fig.align='center'}
tags$div(style="width:500px; margin:0 auto; font-family:Roboto; font-size:50%",
         DiagrammeR::grViz('scripts/ivxyu.gv', width=500, height=250, engine = 'twopi'))
```


Such models were commonly fit with a technique known two-stage least squares, which is demonstrated here.  I use a subset of the <span class="objclass">CigarettesSW</span> data set and <span class="func">ivreg</span> function from the <span class="pack">AER</span> package.
...
Cigarettes = select(CigarettesSW, rprice, rincome, tdiff, tax, cpi, packs, year) %>% 
  filter(year=='1995')

mod0 = lm(log(rprice) ~  log(rincome) + tdiff + I(tax/cpi), data = Cigarettes)
lm(log(packs) ~ fitted(mod0) + log(rincome), data = Cigarettes)
...
mod0 = lm(log(rprice) ~  log(rincome) , data = Cigarettes)
lm(log(packs) ~ fitted(mod0), data = Cigarettes)
