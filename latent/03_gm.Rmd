...
In the graph, $\mathcal{Y}$ is our target variable, $\mathcal{X}$ is the causal variable of interest, and $\mathcal{Z}$ the instrument. The instrumental variable must not be correlated with disturbance of the target variable $\mathcal{Y}$, and only has an indirect relation to it through $\mathcal{X}$.  Neither target nor causal variable are allowed to effect the instrument directly or indirectly.  In graph theory, this involves the notion of conditional independence, where $\mathcal{Z}$ is conditionally independent of Y given X, or in other words, if we remove the X -> Y path, $\mathcal{Z}$ and Y are independent.  Furthemore Z is *not* conditionally independent of X with the removal of that path[^direct].

The motivating factor for such a model is that the simple $\mathcal{X}$ -> $\mathcal{Y}$ path may not provide a means for determining a causal relationship, particularly if $\mathcal{X}$ is correlated with the error. With error $\mathcal{U}$ we can show the following diagram that depicts the problem.
...
To see more clearly where the effect comes from, the coefficient of regressing Y on Z is `r  round(coef(ymod)[2], 3)`, and dividing that by the coefficient of X on Z `r round(coef(mod0)[2], 3)` will provide the ultimate effect of price on the pack of cigarettes that we see above.
