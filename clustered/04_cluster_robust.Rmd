...
library(plm)
t2 = d$time                                      # so plm won't treat time as a factor, time is a reserved name in plm
...
```{r seComparison}
vcovHC(clusterrob_mod, method='arellano', cluster='group') %>% diag %>% sqrt      # plm
(xvar %*% Reduce(`+`, gd$xvar) %*% xvar) %>% diag %>% sqrt                        # custom output

vcov(lm_mod) %>% diag %>% sqrt                                                    # original lm
vcovHC(lm_mod, type='HC0') %>% diag %>% sqrt                                      # non-clustered 'robust' se
```
